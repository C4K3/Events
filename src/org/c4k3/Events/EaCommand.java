	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if ( args.length > 0 ) {
			...
			if ( !arg.equals("true") && !arg.equals("false") ) {
				if ( player == null ) Events.instance.getLogger().info("Invalid argument. Proper syntax is: /ea [true/false]");
				else player.sendMessage(ChatColor.RED + "Invalid argument. Proper syntax is: " + ChatColor.AQUA + "/ea [true/false]");
				return true;
			}

			if ( arg.equals("true") ) newState = true;
			else if ( arg.equals("false") ) newState = false;

		}

		/* Here we toggle the state as there were no arguments */
		...
		String message;
		if ( newState ) message = "Event has been made active.";
		else message = "Event has been made inactive.";
		...
		if ( args.length > 1 ) {
			...
			for ( int i = 1; i < args.length; i++ ) sargs += " " + args[i];

			Events.instance.getServer().broadcastMessage(ChatColor.LIGHT_PURPLE + 
					"[Announcement] Starting event:" + sargs // sargs will have an extra space at the beginning, so we don't need to add one here
					+ "\n Type /event to join."
